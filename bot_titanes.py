...
TOKEN = "7174911284:AAEBErndS8AOgszlMKiiBYJc9CanYdDzGOk"
    ...
    try:
        ...
        if isinstance(maelstroms, list):
            ...
            if maelstrom_vulnerable:
                ...
                total_hearts = maelstrom_vulnerable.get("totalHearts", 1)  # Asegúrate de evitar división por cero
                ...
                progress_formatted = format(progress_percentage, ".2f")

                message += "\n\n🟢 Maelstrom Completamente Vulnerable \n"
                message += f"Nombre: {name}\n"
                message += f"Sistema: {system_name}\n"
                message += f"Corazones Restantes: {hearts_remaining}\n"
                message += f"Progreso de Corazon: {progress_percentage}%\n"
